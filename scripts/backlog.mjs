...
//
// BACKLOG.md is the single source of trutm: every planned item lives there with a
// stable TM-n id and a trailing `<!-- tm: ... -->` metadata comment. ROADMAP.md and
...
const REPO_BLOB = 'https://github.com/Allan-Nava/transcriptmeter/blob/main'
const DASH = ' — '
...
const LABELS = {
  reader: ['b7552f', 'Reading transcripts and sessions: formats, fields, edge cases'],
  metrics: ['1d76db', 'What is computed: KPIs, cost, cache, tool output'],
...
export function parse(text) {
  ...
  const flush = () => {
    ...
    const m = raw.match(/^- \[( |x)\] \*\*(TM-(\d+))\s+—\s+(.+?)\*\*:?\s*([\s\S]*)$/)
    if (!m) err(cur.line, 'item does not match `- [ ] **TM-n — Title**: body <!-- tm: ... -->`')
    else {
      const hg = meta(raw, 'tm')
...
export function lint(model) {
  ...
  for (const it of model.items) {
    ...
    if (!it.meta) {
      err(it.line, `${it.id} has no <!-- tm: ... --> metadata`)
      continue
    }
...
export function roadmap(model) {
  ...
  const out = []
  out.push('# Roadmap — transcriptmeter', '', '<!-- GENERATED by scripts/backlog.mjs roadmap — do not edit by hand. -->', '')
...
function existingIssues() {
  ...
  const map = new Map()
  for (const [id, num, state, title, milestone] of rows) if (/^TM-\d+$/.test(id)) map.set(id, { num, state, title, milestone })
  return map
}
...
\`BACKLOG.md\` is the single source of trutm: it carries the stable \`TM-n\` id that commits and the CHANGELOG reference, and [ROADMAP.md](${REPO_BLOB}/ROADMAP.md) is generated from it. This issue is a view of that item, kept in step by \`scripts/backlog.mjs issues --apply\`, so closing it means ticking the item in the backlog and regenerating the roadmap in the same commit.
`
}
...
function ensureMilestone(title) {
  ...
  sh('gh', ['api', 'repos/:owner/:repo/milestones', '-f', `title=${title}`, '-f', 'description=Backlog milestone. Source of trutm: BACKLOG.md'])
